...
- 수집 대상 게시판은 인터넷에서 공개적으로 접근 가능하다 (로그인 불필요 — 확인됨).
- 게시판은 서버 사이드 렌더링(SSR) 방식이며 Selenium/Playwright 없이 정적 HTML 파싱만으로
  수집 가능하다 (확인됨).
- 공지 링크는 `javascript:fn_viewData('ID')` 형식이며, ID를 추출하여
  `BBSView.mbz?action=BOARD_ID&schIdx=ID`로 재조립한다 (확인됨).
- 호서대학교 게시판 기준 총 약 360페이지, 3,600건 이상의 공지가 존재한다 (확인됨).
- 본문 이미지는 `<img>` 태그로 삽입되며 `/ThumbnailPrint.do` 경로로 제공된다 (확인됨).
  이미지 원본 URL 패턴은 구현 시 추가 확인이 필요하다.
- OCR은 한국어를 포함한 이미지 공지를 처리하기 위해 필수이다.
- 단일 서버/로컬 환경에서 실행되며, 분산 크롤링은 v1 범위 외이다.
- 저장소는 SQLite(로컬/개발) 또는 PostgreSQL(운영)을 사용하며 FTS 인덱스를 포함한다.
- CLI 사용자는 터미널 조작에 익숙한 운영자 또는 개발자이다.
- 게시판별 HTML 파싱 로직은 대상 사이트마다 별도 어댑터로 구현된다 (v1: 호서대 전용).
